...
output "vm_facts" {
  ...
  value = {
    for k, v in azurerm_linux_virtual_machine.vm : k => {
      ...
      # Azure carries a 'location' (region) and an optional 'zones' list; this
      # module does not pin zones, so 'zone' here is the region. Patroni roles
      # can use it as a coarse failover-domain hint.
      zone              = v.location
      instance_type     = v.size
